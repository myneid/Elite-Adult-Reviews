  ...
  class Base
    ...
    def self.searchable_fields(tables = nil)
      ...
      if @searchable_fields.nil?
        ...
        fields = []
        string_columns = self.columns.select { |c| c.type == :text or c.type == :string or c.type == :integer }
    ...
    #
    #   returns count, objs - count is total count (not limit)
    #
    def self.search(text = nil, options = {})
      ...
      if (options[:limit] || options[:offset])
        difference = search_options.keys - [:limit, :offset, :order, :select]
        diff = {}
        difference.each { |k| diff[k] = search_options[k] }
      #  results = [count(diff), find(:all, search_options)]
      else
      #  objs = find(:all, search_options)
      #  results = [objs.size, objs]
      end

      #results
      return find(:all, search_options)
    end
